...
function getResend(): Resend | null {
  ...
  // Dynamic import avoided — constructor is safe when key is real
  const { Resend: ResendClass } = require("resend") as { Resend: new (key: string) => Resend };
  _resend = new ResendClass(key);
  return _resend;
}
